...
public class LeaveServiceImpl implements LeaveService
{
    ...
    public ProcessInstance startWorkflow(LeaveApply apply, String userid, Map<String, Object> variables)
    {
        ...
        apply.setProcess_instance_id(instanceid);// 设置实例id
    ...
    {
        List<LeaveApply> results = new ArrayList<>();
         // * ACT_RU_TASK 连接 ACT_RU_IDENTITYLINK
        //     * ACT_RU_TASK: NAME_ = 调整申请  ASSIGNEE_ = admin
        //     *  ACT_RU_IDENTITYLINK: USER_ID_ = admin
        ...
        for (Task task : tasks)
        {
            ...
            ProcessInstance ins = runtimeservice.createProcessInstanceQuery().processInstanceId(instanceid)
                    .singleResult();
            // 获取 业务主键 act_ru_execution 运行时流程执行实例表 连接 ACT_RU_TASK
            String businesskey = ins.getBusinessKey();
            LeaveApply a = leavemapper.getLeaveApply(Integer.parseInt(businesskey));
            a.setTask(task);
            results.add(a);
